...
<div class="container mx-auto px-4 py-8 fondo-gris rounded-xl">
    ...
    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Ofertas Recibidas</h1>
            <p class="text-sm text-gray-500 mt-1">{{ $offers->total() }} ofertas recibidas</p>
        </div>
    </div>
...
<div class="mb-6">
    <div class="flex flex-wrap gap-2 mb-4">
        ...
        <a href="{{ route('dashboard.received-offers', ['status' => 'pending', 'filter' => $filter, 'sort' => $sort]) }}" 
        ...
        <a href="{{ route('dashboard.received-offers', ['status' => 'waiting_payment', 'filter' => $filter, 'sort' => $sort]) }}" 
        ...
        <!-- Aceptadas -->
        <a href="{{ route('dashboard.received-offers', ['status' => 'accepted', 'filter' => $filter, 'sort' => $sort]) }}" 
        ...
        <!-- Rechazadas -->
        <a href="{{ route('dashboard.received-offers', ['status' => 'rejected', 'filter' => $filter, 'sort' => $sort]) }}" 
    ...
    <div class="flex flex-wrap gap-3 items-center bg-white p-4 rounded-lg border border-gray-200">
        ...
        <div class="flex flex-wrap gap-2">
            <a href="{{ route('dashboard.received-offers', ['status' => $status, 'filter' => 'match_general', 'sort' => 'desc']) }}" 
            ...
            <a href="{{ route('dashboard.received-offers', ['status' => $status, 'filter' => 'price', 'sort' => 'desc']) }}" 
            ...
            <a href="{{ route('dashboard.received-offers', ['status' => $status, 'filter' => 'interest', 'sort' => 'desc']) }}" 
    ...
    <div class="flex items-center">
        ...
        <div>
            ...
            <p class="text-sm text-yellow-700 mt-1">
                Estas ofertas han sido aceptadas pero están esperando que el ofertante complete el pago de la comisión.
                Una vez que se realice el pago, el trueque se completará automáticamente.
            </p>
        </div>
    </div>
</div>
...
@if ($offers->isEmpty())
    ...
    <div class="bg-white rounded-xl shadow-sm p-8 text-center">
        ...
        <p class="mt-1 text-gray-500">Las ofertas que recibas aparecerán aquí para que puedas revisarlas.</p>
        @elseif($status == 'waiting_payment')
        <h3 class="mt-4 text-lg font-medium text-gray-900">No hay ofertas pendientes de pago</h3>
        <p class="mt-1 text-gray-500">Las ofertas aceptadas que esperen pago aparecerán aquí.</p>
        @elseif($status == 'accepted')
        <h3 class="mt-4 text-lg font-medium text-gray-900">No tienes ofertas aceptadas</h3>
        <p class="mt-1 text-gray-500">Las ofertas que aceptes aparecerán aquí.</p>
        @elseif($status == 'rejected')
        <h3 class="mt-4 text-lg font-medium text-gray-900">No tienes ofertas rechazadas</h3>
        <p class="mt-1 text-gray-500">Las ofertas que rechaces aparecerán aquí.</p>
        @endif
...
@else
        ...
        <div class="bg-white shadow-sm rounded-xl overflow-hidden">
            <div class="grid grid-cols-1 divide-y">
                ...
                @php
                    $hasAcceptedOfferForProduct = \App\Models\Offer::hasAcceptedOfferForProduct($offer->product_requested_id);
                    $isThisOfferAccepted = $offer->status === 'accepted';
                ...
                <div class="p-6 hover:bg-gray-50 transition 
                    @if($hasAcceptedOfferForProduct && !$isThisOfferAccepted) bg-red-50 @endif
                    @if($isThisOfferAccepted) bg-green-50 @endif
                    @if($offer->status === 'waiting_payment') bg-yellow-50 @endif">
                    
                    <!-- Banner para ofertas rechazadas automáticamente -->
                    @if($hasAcceptedOfferForProduct && !$isThisOfferAccepted)
                    <div class="mb-4 p-3 bg-red-100 border border-red-200 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm text-red-700 font-medium">
                                Esta oferta fue rechazada automáticamente porque aceptaste otra oferta para este producto.
                            </span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="mb-4 p-3 bg-yellow-100 border border-yellow-200 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <span class="text-sm text-yellow-700 font-medium">
                                ✅ Oferta aceptada - Esperando pago del ofertante para completar el trueque
                            </span>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex-1">
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-start">
                                ...
                                <!-- Tu producto (el que recibió la oferta) -->
                                <div class="flex-1">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            @if($offer->productRequested)
                                                ...
                                                <p class="text-xs text-gray-500">Condición: {{ $offer->productRequested->condition_name ?? '-' }}</p>
                                            @else
                                ...
                                <!-- Icono de oferta (flecha hacia ti) -->
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center">
                                        ...
                                        <div class="ml-4">
                                            <h4 class="font-medium text-gray-900">Te ofrecen</h4>
                                            @if($productsOfferedCount > 0)
                                                ...
                                                <p class="text-xs text-gray-500">De: {{ $offer->fromUser->name ?? 'Usuario desconocido' }}</p>
                                            @else
                                                <span class="text-sm text-red-500">Producto no disponible</span>
                                            @endif
                                        </div>
    ...
    <path d="M256 32C114.6 32 0 125.1 0 240c0 47.6 21.4 91.2 56.8 126.2C49.4 419 32 480 32 480s85.6-18.4 145.1-55.3c24.7 5.1 50.7 7.8 78.9 7.8 141.4 0 256-93.1 256-208S397.4 32 256 32z"/>
        ...
        <form method="POST" action="{{ route('offers.reject', $offer) }}">
            @csrf
            <button type="submit"
                    class="w-full inline-flex items-center justify-center px-3 py-2 bg-red-600 border border-transparent rounded-full font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:ring ring-red-300 transition">
        ...
        <form method="POST" action="{{ route('offers.accept', $offer) }}">
            @csrf
            <button type="submit"
                    onclick="return confirm('¿Estás seguro de que quieres aceptar esta oferta? ' + 
                    'Si el ofertante no ha pagado la comisión, se le solicitará que pague para completar el trueque. ' +
                    'Esto rechazará automáticamente todas las demás ofertas para este producto.')"
                    class="w-full inline-flex items-center justify-center px-3 py-2 bg-green-600 border border-transparent rounded-full font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700 active:bg-green-900 focus:ring ring-green-300 transition">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    @elseif ($offer->status === 'waiting_payment')
        ...
        <span class="inline-flex items-center px-3 py-2 bg-yellow-100 border border-yellow-300 rounded-full font-semibold text-xs text-yellow-800 uppercase tracking-widest">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
    ...
    @elseif ($offer->status === 'accepted' && $offer->chat)
        ...
        <a href="{{ route('chat.show', $offer->chat) }}" 
           class="inline-flex items-center px-3 py-2 bg-blue-600 border border-transparent rounded-full font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 active:bg-blue-900 focus:ring ring-blue-300 transition">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <button onclick="openOfferModal({{ $offer->id }})" 
                          ...
                          VER OFERTA
                                ...
                                <!-- Botón para ver oferta en modal -->
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-4">
                                ...
                                <!-- Indicador de oferta aceptada para este producto -->
                                @if($hasAcceptedOfferForProduct && $isThisOfferAccepted)
                                <span class="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 border border-green-300">
                                    <i class="fa-solid fa-crown mr-1"></i> Oferta Ganadora
                                </span>
                                @endif
                            </div>
                        </div>
        ...
        <div class="sticky top-0 bg-green-600 border-b border-gray-200 px-6 py-4 z-10">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-white mt-1" id="modal-subtitle">Información completa de la propuesta</p>
                </div>
...
<!-- Scripts para el modal corregido -->
<script>
...
function openOfferModal(offerId) {
    ...
    fetch(`/offers/${offerId}/details`)
        ...
        .then(offer => {
            renderOfferModal(offer, modalContent);
        })
        .catch(error => {
...
// Función para renderizar el contenido del modal corregido
function renderOfferModal(offer, modalContent) {
    ...
    document.getElementById('modal-subtitle').textContent = 
        `Oferta de ${offer.from_user?.name || 'Usuario'} - ${new Date(offer.created_at).toLocaleDateString('es-CL')}`;

    modalContent.innerHTML = `
        <!-- Header de resumen CORREGIDO -->
            ...
            <div class="bg-purple-50 border border-purple-200 rounded-xl p-4">
                <div class="flex items-start space-x-3">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-purple-600 mt-2">— ${offer.from_user?.name || 'Usuario'}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <!-- Resumen del Intercambio CORREGIDO -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                    <div class="flex justify-between items-center py-2 border-b border-gray-200">
                        <span class="text-gray-600">Valor referencial del producto solicitado:</span>
                        <span class="font-semibold text-gray-900">${formatChileanCurrency(productRequestedValue)}</span>
                    </div>
                    <div class="flex justify-between items-center py-2 border-b border-gray-200">
                        <span class="text-gray-600">Valor referencial del producto ofrecido:</span>
        ...
        <div class="border-t border-gray-200 pt-6">
            ...
            <div class="space-y-4">
                ${offer.status === 'pending' && !offer.has_accepted_offer_for_product ? `
                    <!-- Botones para ofertas pendientes CORREGIDOS -->
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                 <!-- Preguntas intermedias CORREGIDO - TEXTO COMPLETO -->
                        <a href="/oferta/${offer.id}/intermediar" 
                           class="flex items-center justify-center px-6 py-4 bg-orange-600  border border-transparent rounded-xl font-semibold text-white uppercase tracking-wider hover:from-orange-600 hover:to-orange-700 active:bg-orange-900 focus:ring-2 ring-orange-300 transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl">
                            ...
                            <span class="whitespace-nowrap">Preguntas Intermedias</span>
                                      ...
                                      <!-- Rechazar oferta -->
                        <form method="POST" action="/ofertas/${offer.id}/rechazar" class="w-full">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <button type="submit"
                                    class="w-full flex items-center justify-center px-6 py-4 bg-gradient-to-r from-red-500 to-red-600 border border-transparent rounded-xl font-semibold text-white uppercase tracking-wider hover:from-red-600 hover:to-red-700 active:bg-red-900 focus:ring-2 ring-red-300 transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                                <span class="whitespace-nowrap">Rechazar</span>
                            </button>
                        </form>
                        <!-- Aceptar oferta -->
                        <form method="POST" action="/ofertas/${offer.id}/aceptar" class="w-full">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <button type="submit"
                                    onclick="return confirm('¿Estás seguro de que quieres aceptar esta oferta? Esto rechazará automáticamente todas las demás ofertas para este producto.')"
                                    class="w-full flex items-center justify-center px-6 py-4 bg-gradient-to-r from-green-500 to-green-600 border border-transparent rounded-xl font-semibold text-white uppercase tracking-wider hover:from-green-600 hover:to-green-700 active:bg-green-900 focus:ring-2 ring-green-300 transition-all duration-200 transform hover:scale-[1.02] shadow-lg hover:shadow-xl">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                </svg>
                                <span class="whitespace-nowrap">Aceptar Oferta</span>
                            </button>
                        </form>
                ...
                ${offer.status === 'pending' && offer.has_accepted_offer_for_product ? `
                    <!-- Oferta pendiente pero ya hay una aceptada -->
                    <div class="text-center bg-yellow-50 border border-yellow-200 rounded-xl p-6">
                        <div class="w-12 h-12 bg-yellow-100 rounded-full flex items-center justify-center mx-auto mb-4">
                            <svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                            </svg>
                        </div>
                        <h5 class="font-semibold text-yellow-800 mb-2">Oferta Bloqueada</h5>
                        <p class="text-yellow-700 text-sm">Ya has aceptado otra oferta para este producto</p>
                    </div>
                ` : ''}
                ...
                ${offer.status === 'waiting_payment' ? `
                    ${offer.from_user_id === {{ auth()->id() }} ? `
                        <!-- El usuario que hizo la oferta debe pagar -->
                        <div class="text-center">
                            <a href="/checkout/commission/${offer.id}" 
                    ...
                    ` : `
                        ...
                        <div class="text-center bg-yellow-50 border border-yellow-200 rounded-xl p-6">
                            ...
                            <h5 class="font-semibold text-yellow-800 mb-2">Esperando Pago</h5>
                            <p class="text-yellow-700 text-sm mb-4">El ofertante debe completar el pago de la comisión</p>
                            <a href="/oferta/${offer.id}/intermediar" 
                               class="inline-flex items-center justify-center px-6 py-3 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors font-medium">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span class="whitespace-nowrap">Preguntas Intermedias</span>
